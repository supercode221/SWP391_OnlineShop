...
<html>
    <head>
        ...
        <title>Manage | Blog: ${blog.id}</title>
    ...
    <body>
        ...
        <div class="content1" id="content1">
            <a href="bloglistmanager?type=all"><i class="fa-solid fa-arrow-left fa-xl"></i></a>
            <div class="blogdetail-overall-information">
                <div class="category">
                    <label for="cate"><b>Category: </b></label>
                    <select id="cate">
                        <c:forEach items="${blogCategoryList}" var="cate">
                            <option data-cate-id="${cate.id}" <c:if test="${blog.getCate().id == cate.id}">selected</c:if>>${cate.name}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="status">
                    ...
                    <select id="status">
                        <c:forEach items="${statusList}" var="sta">
                            <option <c:if test="${blog.status == sta}">selected</c:if>>${sta}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
            ...
            <div class="blogdetail-main">
                <div class="side-information">
                    ...
                    <div class="author">
                        <p>${blog.author.authorName}</p>
                    </div>
                </div>
                <form action="blogdetailmanager" method="POST" id="formPost">
                    <c:set var="itemCount" value="1" />
                    <div class="main-information">
                        <div class="post-main-title">
                            <label for="maintitle"><b>Title: </b></label>
                            <textarea id="maintitle" name="mainTitle">${blog.title}</textarea>
                        </div>
                        <div class="post-thumbnail mb-3">
                            <label for="postThumbnailImg"><b>Thumbnail: </b></label>
                            <img id="postThumbnailImg" src="${blog.getBlogImageList().get(0).link}" width="300" style="cursor: pointer;">
                        ...
                        <div class="post-main-content mb-3">
                            ...
                            <textarea id="maincontent" type="text" name="mainContent">${blog.content}</textarea>
                        </div>
                        <c:if test="${not empty blogAttrList}">
                            <c:forEach items="${blogAttrList}" var="blogAttr">
                                <div class="post-subtitle mb-3">
                                    <label for="subtitle1"><b>Sub-Title ${itemCount}: </b></label>
                                    <textarea id="subtitle${itemCount}" type="text" name="${blogAttr.type}">${blogAttr.subTitle}</textarea>
                                </div>

                                <!-- Default no-image upload option -->
                                <c:set var="hasSubImage" value="false" />
                                <c:forEach items="${blogSubMediaList}" var="blogSubMed">
                                    <c:if test="${blogSubMed.subTitleId == blogAttr.id}">
                                        <div class="post-subImage mb-3">
                                            <label for="postThumbnailImg"><b>Sub-Image ${itemCount}: </b></label>
                                            <img id="subImage${itemCount}Preview" src="${blogSubMed.link}" width="300" style="cursor: pointer;">
                                            <input name="${blogSubMed.type}" type="file" id="subImage${itemCount}PreviewInput" accept="image/*" style="display: none;">
                                        </div>
                                        <c:set var="hasSubImage" value="true" />
                                    </c:if>
                                </c:forEach>

                                <!-- If no subimage exists -->
                                <c:if test="${not hasSubImage}">
                                    <div class="post-subImage mb-3">
                                        <label><b>Sub-Image ${itemCount}: </b></label>
                                        <input id="subImage${itemCount}PreviewInput" name="subimage3" type="file" accept="image/*" style="display: none;">
                                        <img class="image-upload" id="subImage${itemCount}Preview" class="preview-image" src="#" width="300">
                                    </div>
                                </c:if>

                                <div class="post-subcontent mb-3">
                                    <label for="maincontent"><b>Content ${itemCount}: </b></label>
                                    <textarea id="content${itemCount}<c:if test="${itemCount == 1}">1</c:if>" type="text" name="content${blogAttr.type}">
                                        <c:if test="${not empty blogAttr.subContent}">${blogAttr.subContent}</c:if>
                                        <c:if test="${empty blogAttr.subContent}">This content is blank!</c:if>
                                        </textarea>
                                    </div>
                        ...
                        <c:choose>
                            <c:when test="${blogAttrListSize == 1}">
                                <div class="post-subtitle mb-3">
                            ...
                            <c:when test="${blogAttrListSize == 2}">
                                <div class="post-subtitle mb-3">
                                    <label for="subtitle3"><b>Sub-Title 3: </b></label>
                                    <textarea id="subtitle3" type="text" name="subTitle3"></textarea>
                                </div>
                            ...
                            <c:when test="${blogAttrListSize == 0}">
                                <div class="post-subtitle mb-3">
        ...
        <script>
            ...
            $(document).ready(function () {
                const saveButton = $('<button id="saveButton" class="btn btn-primary" style="display: none; position: fixed; bottom: 20px; right: 20px; z-index: 1000;">Save</button>');
                ...
                $('textarea, input[type="text"]').on('input change', function () {
                ...
                saveButton.on('click', function (e) {
                    ...
                    $.ajax({
                        url: '${pageContext.request.contextPath}/blogdetailmanager?action=saveChanges',
                        type: 'POST',
                ...
                function callAjax(data) {
                    $.ajax({
                        url: '${pageContext.request.contextPath}/blogdetailmanager',
                        type: 'POST',
                        data: data,
                        success: function (response) {
                            $('#success-popup-save').fadeIn();
                ...
                $("#cate").on('change', function () {
                    let cateId = $(this).find(":selected").attr('data-cate-id');
