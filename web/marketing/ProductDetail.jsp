...
<html>
    <head>
        ...
        <title>Manage | ${product.productName}</title>
        ...
        <div class="content1" id="content1">
            <div class="header-section">
                ...
                <div class="product-name">
                    <h4><b>Detail for: </b>${product.productName}</h4>
                </div>
                <div class="product-star">
                    <p><b>${product.star}</b> <i class="fa-solid fa-star" style="color: #FFD43B;"></i> <i>(Total Feedback : <b>${product.countFeedBack}</b>)</i></p>
                </div>
            </div>
            ...
            <div class="product-detail-section" id="product-section">
                ...
                <div class="overall-informations">
                    ...
                    <div class="overall-information-text">
                        <h5 class="mb-3"><b>ID:</b> ${product.productId}</h5>
                        <h5 class="mb-3"><b>Name:</b> <input id="product-name" type="text" value="${product.productName}" oninput="displaySaveBtn()"></h5>
                        <h5 class="mb-3"><b>Description:</b>
                            <textarea id="product-des" oninput="displaySaveBtn()">${product.des}</textarea>
                        </h5>
                        <h5 class="mb-3"><b>Price:</b> <input 
                                id="product-price" 
                                type="text" 
                                value="<fmt:formatNumber value='${product.productPrice}' pattern='###,###,###' />" 
                        ...
                        <h5 class="mb-3"><b>Category:</b>
                            <select id="cate" onchange="displaySaveBtn()">
                                <c:forEach items="${cateList}" var="c">
                                    <option data-cate-id="${c.id}" <c:if test="${product.subCategoryId == c.id}">selected</c:if>>${c.name}</option>
                                </c:forEach>
                            </select>
                        </h5>
                        <h5 class="mb-3"><b>Status:</b>
                            <select id="status" onchange="displaySaveBtn()">
                                <c:forEach items="${statusList}" var="sta">
                                    <option <c:if test="${product.status == sta}">selected</c:if>>${sta}</option>
                                </c:forEach>
                            </select>
                        </h5>
                        <div class="save-overall-information text-end" style="display: none;">
                            <button onclick="saveChanges(${product.productId})">Save</button>
                        </div>
                    </div>
                    <div class="overall-information-image">
                        <div class="product-thumbnail mb-5">
                            ...
                            <div class="thumbnail-box text-center">
                                <input id="thumbnailInputMain" type="file" accept="image/*" style="display: none;">
                                <img id="thumbnailMain" src="${product.productThumb}" alt="${product.productName}" width="300">
                            </div>
                        </div>
                        <div class="product-subimage-wrapper">
                            <h5><b>Sub-Image</b></h5>
                            <div class="subimage-container" id="subimage-container">
                                <c:forEach items="${imageList}" var="subImage">
                                    <div class="subimage-grid" id="sub-image-${subImage.id}">
                                        <button class="remove-button" onclick="deleteSubImage(${subImage.id})">&times;</button>
                                        <input id="subImage${subImage.id}Input" type="file" accept="image/*" style="display: none;">
                                        <img id="subImage${subImage.id}" src="${subImage.link}" alt="Subimage Preview">
                                    </div>
                                </c:forEach>
                            </div>
                ...
                <div class="detail-attributes">
                    ...
                    <div class="tag-section">
                        ...
                        <div class="tag-wrapper" style="margin-left: 30px;">
                            ...
                            <div class="tag-badge-exist mb-3" id="tag-exist">
                                <c:forEach items="${product.tagList}" var="tagExist">
                                    <span class="badge" style="background-color: ${tagExist.colorCode};">
                                        ${tagExist.name}
                                        <button type="button" class="remove-tag-btn" onclick="removeTag(this, ${tagExist.id})"><b>x</b></button>
                                    </span>
                                </c:forEach>
                            </div>
                            ...
                            <div class="tag-badge-addable">
                                <c:set var="countTag" value="0"/>
                                <c:forEach items="${tagList}" var="tagAddable">
                                    <c:if test="${not empty product.tagList && !product.tagList.contains(tagAddable)}">
                                        <span class="badge" style="background-color: ${tagAddable.colorCode};">
                                            ${tagAddable.name}
                                            <button type="button" class="add-tag-btn" onclick="addTag(this, ${tagAddable.id})"><b>+</b></button>
                                        </span>
                                        <c:set var="countTag" value="${countTag + 1}"/>
                                    </c:if>
                                </c:forEach>
                                <c:if test="${countTag == 0}">
                                    <p style="color: #999999;">Product has already all tags!</p>
                                </c:if>
                            </div>
                        </div>
                    </div>
                    <div class="attributes-section">
                        ...
                        <table class="attributes-table">
                            ...
                            <tbody>
                                <c:forEach items="${attributesList}" var="attr">
                                    <tr>
                                        <td>
                                            ${attr.color.name}
                                            <div class="color-display" style="width: 50px; height: 20px; background-color: ${attr.color.colorCode}; border: 1px solid #ddd; display: inline-block; margin-left: 5px;"></div>
                                        </td>
                                        <td>${attr.size.size}</td>
                                        <td><input type="number" min="1" required="" value="${attr.quantity}" oninput="displaySaveAttrBtn(this)"></td>
                                        <td>
                                            <div class="action-attr d-flex gap-3 justify-content-center">
                                                <button class="save-attr" onclick="saveAttributeChanges(this, ${attr.size.id}, ${attr.color.id})" style="display: none;">Save</button>
                                                <button class="remove-attr" onclick="deleteAttribute(this, ${attr.size.id}, ${attr.color.id})">&times;</button>
                                            </div>
                                        </td>
                                    </tr>
                                </c:forEach>
            ...
            <div id="delete-popup-subImage" class="success-popup">
                <span class="tick-animation">✔</span>
                <p>Save Sub-Image successfully</p>
            </div>
        ...
        <script>
            $(document).ready(function () {
                        ...
                        if (file) {
                            ...
                            reader.onload = function (e) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        $('#success-popup-thumbnail').fadeIn();
                                        setTimeout(function () {
                                            $('#success-popup-thumbnail').fadeOut();
                                        }, 800);
                                    }
                                });
                            };
                ...
                handleImageUploadThumbnail(
                        ...
                        document.getElementById("thumbnailInputMain")
                        );
    ...
    `;
            ...
            function deleteSubImage(subImageId) {
                if (confirm('Are you sure you want to delete this sub-image ?')) {
                    let imageCount = document.querySelectorAll(".subimage-grid").length;
                    console.log(imageCount);

                    if (imageCount <= 1) {
                        alert('Product must have at least 1 Sub-Image!');
                        return;
                    } else {
                        let imageDiv = $('#sub-image-' + subImageId);
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                $('#delete-popup-subImage').fadeIn();
                                setTimeout(function () {
                                    $('#delete-popup-subImage').fadeOut();
                                }, 800);
            ...
            function saveChanges(pid) {
                ...
                let productTruePrice = parseInt(productPrice.replace(/,/g, ''), 10); // Convert to an integer
                let cateId = $('#cate').find(":selected").attr('data-cate-id');
                let status = $('#status').val();
                console.log("CateId: " + cateId + ", Status: " + status + ", Name: " + productName + ", Product Description: " + productDes + ", Price: " + productTruePrice);
                $.ajax({
                    url: '${pageContext.request.contextPath}/productdetailmanager',
                    type: 'POST',
                    data: {
                        action: "saveOverallInformation",
                        productName: productName,
                        productDes: productDes,
                        productTruePrice: productTruePrice,
                        cateId: cateId,
                        status: status,
                        pid: pid
                    },
